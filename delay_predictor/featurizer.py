...
def get_previous_arrrival_delay(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df[["previous_arrival_delay", "previous_origin", "previous_destination"]] = df.groupby("TailNum")[["ArrDelay", "Origin", "Dest"]].shift(1)
    
    mask_unsane_previous_flights = df["previous_destination"]!=df["Origin"]
    
    df[mask_unsane_previous_flights][["previous_arrival_delay", "previous_origin", "previous_destination"]] = np.NaN
